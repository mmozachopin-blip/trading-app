...
if not df_mercato.empty:
    ...
    # REGOLA DI TREND: Sotto EMA 200 = Solo SELL | Sopra EMA 200 = Solo BUY
    ...
    # --- MOTORE DI SIMULAZIONE MONTE CARLO CORRETTO ---
    def esegui_monte_carlo(prezzo_corrente, df, tp_pct, sl_pct, direzione, num_simulazioni=500, passi_futuri=50):
        ...
        # Generazione dei percorsi di prezzo casuali estesi
        ...
        # Gestione di sicurezza per evitare lo 0 fisso e mantenere coerenza di trend/ADX
    ...
    fig.add_trace(go.Candlestick(
        x=df_mercato.index,
        open=df_mercato['open'],
        high=df_mercato['high'],
        low=df_mercato['low'],
        close=df_mercato['close'],
        name='Candele'
    ))
    
    fig.add_trace(go.Scatter(
        x=df_mercato.index, y=df_mercato['SMA_20'], 
        mode='lines', name='SMA 20',
        line=dict(color='sandybrown', width=1.2)
    ))
    
    fig.add_trace(go.Scatter(
        x=df_mercato.index, y=df_mercato['EMA_200'], 
    ...
    st.warning("Caricamento dei dati dal server di Binance in corso...")
